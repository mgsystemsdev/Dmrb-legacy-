...
def board_breach_row_display(item: dict) -> BoardBreachDisplay:
    ...
    # INVARIANT: Every turnover must have a non-empty alert_display
...
def display_status_for_board_item(item: dict) -> str:
    ...
    Uses effective_manual_ready_status when present (computed; respects Vacant Ready
    and only overrides On Notice when date passed). Else lifecycle_phase label.
    Never uses raw availability_status for display, as it can be stale and would
    incorrectly show Vacant Ready units as Vacant Not Ready.
    """
    t = item.get("turnover") or {}
    effective = t.get("effective_manual_ready_status")
    if effective:
        normalized = _MANUAL_STATUS_TO_BOARD_LABEL.get(str(effective).strip().lower())
        if normalized:
            return normalized
        return str(effective)
    return status_label(t.get("lifecycle_phase") or "")
...
def qc_label(tasks: list[dict]) -> str:
    ...
    completed = [t for t in tasks if t["execution_status"] == "COMPLETED"]
    if not completed:
        return "Pending"
    all_confirmed = all(t.get("manager_confirmed_at") is not None for t in completed)
