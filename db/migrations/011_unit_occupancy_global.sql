-- Work Order Validator: one move-in date per unit, property-scoped.
-- Populated via Resident Activity (or any future source) through occupancy_service.ingest().
-- Used by the WO Validator to compute days_since_move_in and classify work orders.
...
COMMENT ON TABLE unit_occupancy_global IS 'Latest known move-in date per unit. One row per (property_id, unit_id). Populated by occupancy_service.ingest() from Resident Activity or other sources.';
