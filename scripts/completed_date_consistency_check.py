...
def main() -> int:
    ...
    print(
        f"  Found {len(rows)} task(s) with execution_status = 'COMPLETED' and completed_date IS NULL:"
    )
    for r in rows:
        print(
            f"    task_id={r['task_id']} turnover_id={r['turnover_id']} task_type={r['task_type']!r} vendor_completed_at={r['vendor_completed_at']}"
        )
