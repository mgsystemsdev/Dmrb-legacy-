...
def _rows_to_df(rows: list[dict]) -> pd.DataFrame:
    ...
    # MO/Confirm
    df["mo_confirm"] = df["confirmed_move_out_date"].apply(lambda v: "Yes" if v is not None else "")
...
def _build_bucket_df(
    ...
    for b in buckets:
        columns[b].sort(key=lambda e: -int(e.split("DV-")[1]) if "DV-" in e else 0)
...
def build_dmrb_report(rows: list[dict], metrics: dict, today: date) -> bytes:
    ...
    _write_entry(
        ws, dash, _SC, _SR, "DashboardVacant", title="Units Overview", fills_fn=_apply_status_fills
    )
    ...
    all_rows = rows
    ready_rows = (
        [r for r, s in zip(rows, status_norm) if s == "VACANT READY"] if not df.empty else []
    )
    not_rdy_rows = (
        [r for r, s in zip(rows, status_norm) if s == "VACANT NOT READY"] if not df.empty else []
    )
    ...
    if not phases:
        ...
        _write_entry(
            ws, empty_grid, _SC, _SR, "ActiveAgingEmpty", title="Active Aging (Vacant Not Ready)"
        )
    ...
    mi_df = (
        df[df["move_in_date"].notna() & ~df["is_unit_ready"].astype(bool)] if not df.empty else df
    )
    mi_df = (
        mi_df.sort_values("move_in_date", ascending=True, na_position="last")
        if not mi_df.empty
        else mi_df
    )
    ...
    sla_df = (
        sla_df.sort_values("dv", ascending=False, na_position="last")
        if not sla_df.empty
        else sla_df
    )
    sla_view = _pick(
    ...
    insp_df = (
        insp_df.sort_values("dv", ascending=False, na_position="last")
    ...
    plan_df = (
        plan_df.sort_values("dv", ascending=False, na_position="last")
    ...
    wpb_data: dict[str, Any] = {
        ...
        "Move Ins \u26a0\ufe0f Alert": vacant_df["attention_badge"].values
    ...
    _write_entry(
        ws, wpb_df, _SC, _SR, "WalkBoard", title="Walking Path Board", fills_fn=_apply_status_fills
    )
    ...
    stalled_df = (
        stalled_df.sort_values("task_completion_ratio", ascending=True, na_position="last")
    ...
    stalled_view = pd.DataFrame(
        {
            ...
            "Progress %": stalled_df["task_completion_ratio"].values
            if not stalled_df.empty
            else [],
            "Execution State": stalled_df["status"].values if not stalled_df.empty else [],
            "\u26a0\ufe0f Alert": stalled_df["attention_badge"].values
            if not stalled_df.empty
            else [],
        }
    )
    ...
    clean_df = (
        df[(df["task_state"] == "All Tasks Complete") & df["is_vacant"]] if not df.empty else df
    )
    ...
    cur_row = _write_entry(
        ws, clean_view, _SC, cur_row, "CleanTurn", title="Clean Turn", fills_fn=_apply_status_fills
    )

    # 3) Task Flow: in-turn-execution AND in progress
    flow_df = (
        df[df["in_turn_execution"] & (df["task_state"] == "In Progress")] if not df.empty else df
    )
    flow_df = (
        flow_df.sort_values("task_completion_ratio", ascending=True, na_position="last")
    ...
    for _key, title, sched_col, _vendor_col, status_col in _SCHED_TASKS:
        if not df.empty:
            ...
            sec = (
                ...
                if False
                else df[df[sched_col].notna()].copy()
            )
            ...
            sec = df[
                ...
                .str.strip()
                .str.lower()
                .isin({"in progress", "scheduled", "in_progress"})
            ].copy()
    ...
    for _key, title, _sched_col, vendor_col, status_col in _SCHED_TASKS:
        if not df.empty:
            sec = df[
                df[vendor_col].apply(lambda d: d is not None and today <= d <= window_end)
            ].copy()
    ...
    # 2) Turn Performance
    vacant_dv = (
        df.loc[df["is_vacant"], "dv"].dropna() if not df.empty else pd.Series([], dtype=float)
    )
    ...
    comp_rate = (vacant_n - tot_breach) / vacant_n * 100 if vacant_n > 0 else 100.0
    sla_df = pd.DataFrame(
        {
            "Metric": [
                "Turn SLA Breaches",
    ...
    if not df.empty:
        state_counts = (
            ...
            .map(_STATE_TO_STATUS)
            .fillna(df["operational_state"])
            .value_counts()
        )
        op_metrics = list(state_counts.index)
        op_values = [f"{cnt} ({cnt / total_u * 100:.1f}%)" for cnt in state_counts.values]
    else:
        op_metrics, op_values = [], []
    ops_status_df = pd.DataFrame({"State": op_metrics, "Units": op_values})
    cur_row = _write_entry(
        ws, ops_status_df, _SC, cur_row, "OperationalStatus", title="Operational Status"
    )
    ...
    cur_row = _write_entry(ws, ready_df2, _SC, cur_row, "ReadyForMoveIn", title="Ready For Move-In")
    ...
    if at_risk_n > 0:
        alerts.append((f"{at_risk_n} units at risk of SLA breach (8-9 days)", "Warning"))
    if tot_breach > 0:
        alerts.append((f"{tot_breach} units currently in SLA breach", "Critical"))
    if mi_breach > 0:
        alerts.append((f"{mi_breach} units at risk of missing move-in", "Critical"))
    if stalled_n > 0:
        alerts.append((f"{stalled_n} units with stalled tasks", "Warning"))
    if not alerts:
    ...
    if not df.empty:
        ...
        def _build_flags(r: Any) -> str:
            ...
            if r["sla_movein_breach"]:
                f.append("MOVE-IN RISK")
            if r["is_task_stalled"]:
                f.append("STALLED")
            if r["has_notes"]:
                f.append(str(r["notes_joined"])[:30])
    ...
    n_top = len(pri_view)
    cur_row = _write_entry(
        ws, pri_view, _SC, cur_row, "TopPriorityUnits", title=f"Top {n_top} Priority Units"
    )
    ...
    cur_row = _write_entry(
        ws, comp, _SC, cur_row, "PhaseComparison", title="Phase Performance Comparison"
    )
    ...
    cur_row = _write_entry(ws, totals, _SC, cur_row, "PortfolioSummary", title="Portfolio Summary")
    ...
    if len(comp) >= 2:
        ...
        performers = pd.DataFrame(columns=["Phase", "Category", "Avg Turn Days", "Compliance Rate"])

    cur_row = _write_entry(
        ws, performers, _SC, cur_row, "Performers", title="Best & Worst Performers"
    )
