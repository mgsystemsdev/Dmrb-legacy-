...
Classification rule (Make Ready unchanged):
...
# Substrings for Service Technician variants (case-insensitive; checked in order)
...
def _matches_unit_pattern(location: str) -> bool:
    """True if Location looks like phase-building-unit (e.g. 4-26-0417)."""
...
def _refine_service_technician_label(location: str) -> str:
    """Map Location to Service Tech label when base classification is Service Technician."""
...
def _classify(days_since: int | None, move_in_date: date | None, unit_found: bool) -> str:
    """Return the WO classification string for a single work order."""
    if not unit_found:
        return "Service Technician"
...
def _extract_phase(location: str) -> str:
    """Return the uppercased phase segment from a Location string."""
    parts = parse_unit_parts(normalize_unit_code(location))
    if parts["phase_code"]:
        return parts["phase_code"].upper()
    # Location may be a common area name with no hyphens
    return normalize_unit_code(location)


def _extract_building(location: str) -> str | None:
    """Return the building segment from a Location string, or None."""
    parts = parse_unit_parts(normalize_unit_code(location))
