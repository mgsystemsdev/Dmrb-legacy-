...
def _detect_engine(filename: str) -> str:
    """Return the pandas Excel engine based on file extension."""
    return "xlrd" if filename.lower().endswith(".xls") else "openpyxl"


def _cell_text(value) -> str:
    """Return stripped string representation of a cell value, or '' for NaN."""
    if pd.isna(value):
        return ""
...
def _build_col_map(df: pd.DataFrame, section_row: int) -> dict[str, int]:
    """
    Scan up to 5 rows after section_row for the MOVE-INS sub-header row.

    Searches each cell in those rows for the search terms in _RA_COLUMNS.
    Returns a dict {field_name: column_index} using either the detected
    index or the fallback_index if detection fails for a field.
    """
    ...
    # Apply fallbacks for any fields not found
...
def _parse_date(value) -> date | None:
    """Coerce a cell value to datetime.date; return None on failure."""
    if pd.isna(value):
        return None
    try:
...
def parse(file_content: bytes, filename: str = "resident_activity.xls") -> list[dict]:
    ...
    # Find all MOVE-INS section start rows
    ...
    for sec_start in section_rows:
        ...
        # Advance past header rows — start reading data 4 rows after section header
        ...
        for row_idx in range(data_start, len(df)):
            ...
            # Skip page-continuation rows
            if "continued from previous page" in col0.lower():
                continue

            # Extract fields using detected (or fallback) column indices
